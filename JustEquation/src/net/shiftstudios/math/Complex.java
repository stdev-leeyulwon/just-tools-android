...
public final class Complex implements Serializable {
	...
	public String toString() {
    	if (m(re) !=0) {
    	if (m(im) > 0) {
    		return Double.toString(m(re)) + " + " + Double.toString(m(im)) + "i";
    	} else if (im ==0) {
    		return Double.toString(m(re));
    	} else if (im <0) {
    		return Double.toString(m(re)) + " - " + Double.toString(m(-im)) + "i";
    	}
    	} else if (m(re) ==0) {
    		if (m(im) !=0) {
    		return Double.toString(m(im)) + "i";
    		} else if (m(im) ==0) {
    			return "0";
    		}
    	}
    ...
    public double m(double d){
    	if (Math.abs(d - Math.rint(d)) < 0.001) {
    		...
    		return Math.rint(d * 10000000)/10000000;
    	}
	}
