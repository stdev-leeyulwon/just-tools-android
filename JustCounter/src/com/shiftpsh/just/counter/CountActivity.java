...
import com.shiftpsh.just.counter.R;
...
public Counter[] counter_data = new Counter[32];
...
public String curCounterName = "Counter 1";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		m.setOnTouchListener(new RelativeLayout.OnTouchListener(){
		    ...
		    adapter = new CounterAdapter(this, 
		                R.layout.drawer_item_row, counter_data);
	...
	public void onCounterUpdate(){
		if(curCounterName.equals("Counter "+(curCounter+1))){
			counter_data[curCounter] = new Counter(i, "Counter "+(curCounter+1));
		} else {
	...
	private class DrawerItemClickListener implements ListView.OnItemClickListener {
		...
		public void onItemClick(AdapterView parent, View view, int position, long id) {
			if (position == 0) {
				if (counters < 32){
					counters++;
					counter_data[counters - 1] = new Counter(0, "Counter "+counters);
				} else {
					Toast.makeText(getApplicationContext(), "Total amount exceeded!", Toast.LENGTH_SHORT).show();
				}
			} else {
